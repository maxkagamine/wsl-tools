...
1. エクスプローラーからVS Codeを起動する際（ファイルをダブルクリックするか、コンテキストメニューの「<img src="https://code.visualstudio.com/assets/apple-touch-icon.png" height="16" align="center" /> Codeで開く」を選択する場合）、WSLで開くようにします。これは[小さなプログラム](src/bin/run-in-wsl.rs)を使用してパス変換を行い、WSL内の`code`シェルスクリプトと同じようにVS Codeを起動します。
